...
```{r}
co_water_data <- function(site, startw_year, endw_year){
raw_water_data <- GET(url = paste0("https://dwr.state.co.us/Rest/GET/api/v2/surfacewater/",
                  ...
                  "&min-measDate=10%2F01%2F", startw_year - 1, 
                  "&max-measDate=09%2F30%2F", endw_year))

extracted_water_data <- httr::content(raw_water_data, as = "text", encoding = "UTF-8") 

final_water_data <- jsonlite::fromJSON(extracted_water_data) 

return(final_water_data$ResultList)

}
#test
testytest <- co_water_data(site = "CLAFTCCO", startw_year = 2000, endw_year = 2001 )
...
map_co_water_data<- sites %>% map(~co_water_data(., startw_year = 2021, endw_year = 2022)) 
multi_sites <-bind_rows(map_co_water_data)
view(multi_sites)
...
combo_<-bind_rows(mut_co_water_data, mut_usgs)
...
```
According to the data there are 105 days where the canyon mouth has LOWER flow. I am not sure what I was expecting, but it seems logical that upstream locations have less water, and it pools at the lower portions so there should be more water?????
