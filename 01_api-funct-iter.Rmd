...
```{r}
parkwide_visitation <- function(year =2018){

packed_data <-GET(url= paste0("https://irmaservices.nps.gov/v3/rest/stats/total/",year))

unpacked_data <- httr::content(packed_data, as ="text", encoding= "UTF-8") 
...
Data_2018 <-parkwide_visitation(year= 2018)
Data_1999<-parkwide_visitation(year=1999)
Data_1980<-parkwide_visitation(year=1980)
...
```{r}
raw_datarocky <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=1&startYear=2010&endMonth=12&endYear=2021")

unpacked_data_rocky <- httr::content(raw_data, as = "text", encoding = "UTF-8")

final_data_rocky <- jsonlite::fromJSON(unpacked_data)
...
```{r}
unit_visitation <- function(parks,start_month, start_year,end_month,end_year){
...
extracted_data_rocky <- httr::content(raw_datarocky, as = "text", encoding = "UTF-8") 

final_data_rocky <- jsonlite::fromJSON(extracted_data_rocky) 
...
```{r}
#for loops
pull_ROMO <- unit_visitation(park = "ROMO", start_month = 1, start_year = 1990, end_month = 12, end_year = 2021) 

pull_EVER <- unit_visitation(park = "EVER", start_month = 1, start_year = 1990, end_month = 12, end_year = 2021) 

pull_THRO <- unit_visitation(park = "THRO", start_month = 1, start_year = 1990, end_month = 12, end_year = 2021)
...
```{r}
unit_visitation <- function(parks,start_month="1", start_year,end_month="12",end_year){
raw_datarocky <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=",parks, "&startMonth=", start_month,"&startYear=", start_year,"&endMonth=", end_month, "&endYear=", end_year))
...
rockymountain<-unit_visitation(park="ROMO", start_year=1980, end_year=2021)
everglades<-unit_visitation(park="EVER",start_year=1980, end_year=2021)
roosevelt<-unit_visitation(park="THRO", start_year=1980, end_year=2021)
...
#view(output_parksloop), Error in (function (..., row.names = NULL, check.rows = FALSE, check.names = TRUE,  : arguments imply differing number of rows: 0, 504
...
output_parksloop[[i]]<- unit_visitation(park=parks[i])
}

output_map_parks <-parks %>%
  map(~unit_visitation(parks= .))
multi_map_parks <- output_map_parks %>%
  bind_rows()
